...
func HandleListWithFilterSuccessfully(t *testing.T, fakeServer th.FakeServer) {
	fakeServer.Mux.HandleFunc("/v2.0/ports", func(w http.ResponseWriter, r *http.Request) {
		th.TestMethod(t, r, "GET")
		th.TestHeader(t, r, "X-Auth-Token", fake.TokenID)

		// Check if the filter parameter is present
		if r.URL.Query().Get("propagate_uplink_status") != "true" {
			t.Errorf("Expected propagate_uplink_status=true in query, got: %s", r.URL.RawQuery)
		}

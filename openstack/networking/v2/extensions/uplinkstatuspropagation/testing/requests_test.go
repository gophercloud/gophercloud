...
func TestListWithFilter(t *testing.T) {
	...
	HandleListWithFilterSuccessfully(t, fakeServer)
	...
	allPages, err := ports.List(fake.ServiceClient(fakeServer), listOpts).AllPages(context.TODO())
	th.AssertNoErr(t, err)
	...
	if actual[0].PropagateUplinkStatus != nil {
		th.AssertEquals(t, *actual[0].PropagateUplinkStatus, true)
	}
}

...
Example to Update a L7Policy
	...
	l7policy, err := l7policies.Update(networkClient, l7policyID, updateOpts).Extract()
	if err != nil {
		panic(err)
	}
...
Example to List L7Policy Rules
	...
	allPages, err := l7policies.ListRules(networkClient, l7policyID, listOpts).AllPages()
	if err != nil {
		panic(err)
	}
...
Example to Create a Rule
	...
	rule, err := l7policies.CreateRule(networkClient, l7policyID, createOpts).Extract()
	if err != nil {
		panic(err)
	}
...
Example to Update a Rule
	...
	rule, err := l7policies.UpdateRule(networkClient, l7policyID, ruleID, updateOpts).Extract()
	if err != nil {
		panic(err)
	}
...
Example to Delete a Rule
	...
	err := l7policies.DeleteRule(networkClient, l7policyID, ruleID).ExtractErr()
	if err != nil {
		panic(err)
	}
*/

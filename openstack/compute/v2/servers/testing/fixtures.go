...
// HandleBadNetworkAddressListSuccessfully sets up the test server to respond
// to a bad ListAddressesByNetwork request.
// The IP versions are strings instead of numbers.
func HandleBadNetworkAddressListSuccessfully(t *testing.T) {
	th.Mux.HandleFunc("/servers/asdfasdfasdf/ips/public", func(w http.ResponseWriter, r *http.Request) {
		th.TestMethod(t, r, "GET")
		th.TestHeader(t, r, "X-Auth-Token", client.TokenID)
...
// HandleBadCreateServerImageSuccessfully sets up the test server to respond to
// a bad TestCreateServerImage request.
// The location header is missing.
func HandleBadCreateServerImageSuccessfully(t *testing.T) {
	th.Mux.HandleFunc("/servers/serverimage/action", func(w http.ResponseWriter, r *http.Request) {
		th.TestMethod(t, r, "POST")
		th.TestHeader(t, r, "X-Auth-Token", client.TokenID)
		w.WriteHeader(http.StatusAccepted)
...
// HandleBadCreateServerImageURLSuccessfully sets up the test server to respond to
// a bad TestCreateServerImage request.
// The location header is incorrect.
func HandleBadCreateServerImageURLSuccessfully(t *testing.T) {
	th.Mux.HandleFunc("/servers/serverimage/action", func(w http.ResponseWriter, r *http.Request) {
		th.TestMethod(t, r, "POST")
		th.TestHeader(t, r, "X-Auth-Token", client.TokenID)
		w.Header().Add("Location", "https://0.0.0.0/images/.")
		w.WriteHeader(http.StatusAccepted)
	})
}
...
// IDFromNameServerListResults contains the canned body of a servers.List response.
const IDFromNameServerListResults = `
{
	"servers": [
		{
			"status": "ACTIVE",

...
func Get(ctx context.Context, client *gophercloud.ServiceClient, serverID, portID string) (r GetResult) {
	resp, err := client.GetWithContext(ctx, getInterfaceURL(client, serverID, portID), &r.Body, &gophercloud.RequestOpts{
		OkCodes: []int{200},
	})
...
func Create(ctx context.Context, client *gophercloud.ServiceClient, serverID string, opts CreateOptsBuilder) (r CreateResult) {
	...
	resp, err := client.PostWithContext(ctx, createInterfaceURL(client, serverID), b, &r.Body, &gophercloud.RequestOpts{
		OkCodes: []int{200},
	})
	_, r.Header, r.Err = gophercloud.ParseResponse(resp, err)
	return
...
func Delete(ctx context.Context, client *gophercloud.ServiceClient, serverID, portID string) (r DeleteResult) {
	resp, err := client.DeleteWithContext(ctx, deleteInterfaceURL(client, serverID, portID), nil)
	_, r.Header, r.Err = gophercloud.ParseResponse(resp, err)
	return
}

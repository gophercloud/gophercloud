...
func Delete(ctx context.Context, client *gophercloud.ServiceClient, id string) (r DeleteResult) {
	resp, err := client.DeleteWithContext(ctx, deleteURL(client, id), &gophercloud.RequestOpts{
		OkCodes: []int{200},
	})
	_, r.Header, r.Err = gophercloud.ParseResponse(resp, err)
	return
...
func Complete(ctx context.Context, client *gophercloud.ServiceClient, id string) (r CompleteResult) {
	...
	resp, err := client.PostWithContext(ctx, completeURL(client, id), b, nil, &gophercloud.RequestOpts{
		OkCodes: []int{204},
	})
	_, r.Header, r.Err = gophercloud.ParseResponse(resp, err)
	return

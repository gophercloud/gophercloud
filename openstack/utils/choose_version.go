...
func ChooseVersion(ctx context.Context, client *gophercloud.ProviderClient, recognized []*Version) (*Version, string, error) {
	...
	var resp response
	_, err := client.RequestWithContext(ctx, "GET", client.IdentityBase, &gophercloud.RequestOpts{
		JSONResponse: &resp,
		OkCodes:      []int{200, 300},
	})
...
func GetSupportedMicroversions(ctx context.Context, client *gophercloud.ServiceClient) (SupportedMicroversions, error) {
	...
	var resp response
	_, err := client.GetWithContext(ctx, client.Endpoint, &resp, &gophercloud.RequestOpts{
		OkCodes: []int{200, 300},
	})

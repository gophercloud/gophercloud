...
func (r commonResult) Extract() (*Node, error) {
	...
	err := r.ExtractInto(&s)
	if err != nil {
		return s.Node, fmt.Errorf("Error extracting node. %v", err)
	}

	actionID, errAction := r.ExtractAction()
	if errAction == nil {
		s.Node.ActionID = actionID
	}

	return s.Node, err
}

func (r commonResult) ExtractAction() (string, error) {
	if len(r.Header) > 0 {

...
This branch contains the current stable branch of Gophercloud: `v2`. The legacy stable version can be found in the `v1` branch.
...
* `ProviderClient` holds the authentication token and can be used to build a
  `ServiceClient`;
* `ServiceClient` specializes agains one specific OpenStack module and can
...
providerClient, err := openstack.AuthenticatedClient(context.TODO(), opts)
if err != nil {
    panic(err)
}
...
```go
providerClient, err := openstack.AuthenticatedClient(context.TODO(), gophercloud.AuthOptions{
